...
//	@Test // should probably have its own test class.
	...
	public void searchMetadata(){
		client.lock(pgDoc);
		assert client.setMeta(pgDoc, "<meta><name>Alice</name></meta>");
		client.unlock(pgDoc);
	...
	public void searchMetadataWithCondition(){
		...
		client.lock(pgDoc);
		assert client.setMeta(pgDoc, "<meta><name>Smaug</name></meta>");
		client.unlock(pgDoc);
		xml = client.searchObjects(query);
		checkResultSize(xml,1);
	}
	...
	public void searchMetadataWithSpanQuery(){
		client.lock(pgDoc);
		assert client.setMeta(pgDoc, "<meta><name>Alice in Wonderland</name></meta>");
		client.unlock(pgDoc);
	...
	public void searchMetadataFoxSpanQuery(){
		...
		client.lock(pgDoc);
		assert client.setMeta(pgDoc, "<meta><name>The sick brown fox dies alone.</name></meta>");
		client.unlock(pgDoc);
	...
	public void searchUserQueryQuotedString(){
		client.lock(pgDoc);
		assert client.setMeta(pgDoc, "<meta><name>Cinnamon meets Lucene - happy?.</name></meta>");
		client.unlock(pgDoc);
	...
	public void searchMetadataUmlauts(){
		client.lock(pgDoc);
		assert client.setMeta(pgDoc, "<meta><name>Ülüce</name></meta>");
		client.unlock(pgDoc);
	...
	public void searchAttributes(){
		client.lock(pgDoc);
		assert client.setMeta(pgDoc, "<meta><test attrib='a special attribute'/></meta>");
		client.unlock(pgDoc);
	...
	public void searchDecimal(){
		client.lock(pgDoc);
		assert client.setMeta(pgDoc, "<meta><test><dec>12.34</dec></test><test><dec>43.21</dec></test></meta>");
		client.unlock(pgDoc);
...
//		client.reindex("folders");
		...
		fields.put("metadata", "<meta><description>a funny folder</description></meta>");
